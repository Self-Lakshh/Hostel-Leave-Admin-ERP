...
type SecurityCardProps = {
    ...
    onReactivate: (guard: SecurityGuard) => void
}
...
const SecurityCard = ({ guard, onEdit, onDelete, onReactivate }: SecurityCardProps) => {
    const isActive = guard.active
            ...
            <div className={`flex flex-col flex-1 transition-opacity duration-200 ${!isActive ? 'opacity-40' : ''}`}>
                <div className="p-5 flex flex-col gap-4 flex-1">
                    ...
                    <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0 ${getAvatarBg(guard._id)}`}>
                            {getInitials(guard.name)}
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="font-semibold text-gray-900 text-[14px] truncate leading-tight">{guard.name}</p>
                            <p className="text-[11px] text-gray-400 font-mono mt-0.5">{guard.emp_id}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1 flex-shrink-0">
                            <span className="text-[11px] text-gray-500 font-medium">Security</span>
                    ...
                    <div className="flex flex-col gap-2.5">
                        <InfoRow icon={<Phone size={13} />} text={guard.phone_no} />
                        <InfoRow icon={<Mail size={13} />} text={guard.email} />
                    </div>
                </div>
            </div>
